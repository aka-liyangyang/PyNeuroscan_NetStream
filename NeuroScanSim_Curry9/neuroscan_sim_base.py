...
class NeuroScanSimulatorServer:
    ...
    def _send_keepalive(self, client: ClientState) -> None:
        ...
        client.socket.sendall(packet)
        self.log(f"[{self.mode}] keepalive sent sample_index={client.sample_index}")
    ...
    def _send_eeg(self, client: ClientState) -> None:
        ...
        client.socket.sendall(packet)
        client.eeg_packet_count += 1
        preview = data[:2, :6].tolist()
        self.log(
            f"[{self.mode}] EEG packet #{client.eeg_packet_count} sent "
            f"start_sample={client.sample_index} shape={data.shape} preview={preview}"
        )
